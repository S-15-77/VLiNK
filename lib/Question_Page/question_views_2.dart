...
class QuestionViews2 extends StatefulWidget {
  QuestionViews2(
      {required this.ansQnsIndex,
      required this.ans,
      required this.ansQns,
      required this.markSub,
      required this.flaggedQns});
  var ansQnsIndex;
  final markSub;
  ...
  @override
  _QuestionViews2State createState() => _QuestionViews2State(
      ansQnsIndex: ansQnsIndex,
      markSub: markSub,
      ans: ans,
      ansQns: ansQns,
      flaggedQns: flaggedQns);
}

class _QuestionViews2State extends State<QuestionViews2> {
  _QuestionViews2State(
      {required this.ansQnsIndex,
      required this.ans,
      required this.ansQns,
      required this.markSub,
  ...
  Future<List<Questions>> getData(ansQnsIndex) async {
    return db.fetchQuestionsList(ansQnsIndex);
  }
  ...
  @override
  void initState() {
    _questions = getData(ansQnsIndex);
    starttimer();
  }

  bool canceltimer = false;
  late int timer = Provider.of<addValues>(context, listen: false).timer;
  late String showtimer =
      "${Provider.of<addValues>(context, listen: false).timer}";
    ...
    Timer.periodic(onesec, (Timer t) {
      setState(() {
        if (timer < 1) {
          ...
          //     context,
          //     MaterialPageRoute(
          //         builder: (context) => QuestionOverviews(
          //               flagList: flaggedQns,
          //               ansList: ansQns,
          //             )));
        ...
        Provider.of<addValues>(context, listen: false).getTimer(timer);
      });
    });
  }
  ...
  void checkAnswerAndUpdate(bool value, String Subject) {
    if (isAlreadySelected) {
      ...
      if (value == true) {
        print("true");
        print("Before $flaggedQns");
        flaggedQns.remove(ansQnsIndex);
        print(flaggedQns);
        ansQns.add(ansQnsIndex);
        print("ANsqns: $ansQns");
        // ans.removeAt(ansQnsIndex);
        // ans.insert(ansQnsIndex, 1);
        ans[ansQnsIndex] = 1;
        print("ANS: $ans");
        //score++;
        ...
        print("fls");
        // flaggedQns.removeAt(index);
        // ans.removeAt(ansQnsIndex);
        // ans.insert(ansQnsIndex, 0);
        ans[ansQnsIndex] = 0;
        flaggedQns.remove(ansQnsIndex);
        print("else flagged :$flaggedQns");
        ansQns.add(ansQnsIndex);
        print("ANsqns: $ansQns");
        print("ANS: $ans");
        setState(() {
